...
public class _03_MurderOfCrows {
	...
	private void findTheDiamond() {
		...
		for (int i = 0; i < theMurder.size(); i++) {
			Crow suspect = theMurder.get(i);
			System.out.println(suspect.getName() + " has a terrible secret");
			for (int j = 0; j < suspect.getStomachContents().size(); j++) {
				if (suspect.getStomachContents().get(j) == "diamond") {
					System.out.println(suspect.getName() + " is Guilty!");
					System.out.println(suspect.getName() + ": HOW DID YOU KNOW IT WAS ME");
					System.out.println("With array manipulation");
					System.out.println("Sadly, " + crowsdead + " crows died before I found you.");
					System.exit(0);
				} else {
					System.out.println("ewwwwwww... its a " + suspect.getStomachContents().get(j));
				}
							}

			System.out.println("-Another crow dead, a criminal still on the loose-");
			crowsdead++;
		}
